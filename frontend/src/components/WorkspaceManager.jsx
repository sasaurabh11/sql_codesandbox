          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="bg-gray-700">
                ...
                {table.columns.map((c, ci) => (
                  <th
                    key={ci}
                    className="border-r border-gray-600 px-3 py-3 text-left last:border-r-0"
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      {c.primaryKey && (
                        <span
                          className="text-xs bg-yellow-500 text-yellow-900 px-1 rounded"
                          title="Primary Key"
                        >
                          PK
                        </span>
            ...
            <tbody>
              {table.rows.slice(0, currentVisibleRows).map((row, ri) => (
                <tr
                  key={ri}
                  className="border-t border-gray-600 hover:bg-gray-650 transition-colors group"
                >
                  ...
                  {row.map((cell, ci) => (
                    <td
                      key={ci}
                      className="border-r border-gray-600 px-3 py-2 last:border-r-0"
                    >
                      <div className="flex items-center space-x-2">
                        <span
                          className={
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 p-3 bg-gray-700 rounded-lg">
          <div className="text-sm text-gray-400">
            ...
            <span className="text-blue-400 font-semibold">
              {Math.min(currentVisibleRows, totalRows)}
            </span>{" "}
            of <span className="text-green-400 font-semibold">{totalRows}</span>{" "}
            rows •{" "}
      ...
      {workspaceData && (
        <div className="mt-6 bg-gray-750 border border-gray-600 rounded-xl p-4">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-sm text-gray-400">
              {workspaceData.tables.length} table
              {workspaceData.tables.length !== 1 ? "s" : ""}
            </div>
          </div>
          ...
          <div className="space-y-4">
            {workspaceData.tables.map((tbl, i) => (
              <div
                key={i}
                className="border border-gray-600 rounded-lg bg-gray-750 hover:bg-gray-700 transition-colors overflow-hidden"
              >
                <div
                  ...
                  <div className="flex items-center space-x-3">
                    <svg
                      className={`w-5 h-5 text-blue-400 transition-transform ${
                        expandedTables.has(i) ? "rotate-90" : ""
                      }`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
